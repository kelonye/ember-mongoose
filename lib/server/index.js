...
Api.prototype.all = function(req, res) {
  var that = this;
  if (req.query){
    var c = req.query.c; // conditions
    var f = null; // this.pathsString; // fields
    var o = req.query.o; // options
  }
  that.model.find(c, f, o, function(err, items) {
...
/**
  * Use `plugin`
  */
Apis.prototype.use = function(path, plugin) {
...
Apis.prototype.getURIS = function() {
  ...
  Object.keys(that).forEach(function(name){
    if (name == 'models' || name == 'plugins') return;
    app.use(that[name].getURIS());
  });

  Object.keys(that.plugins).forEach(function(path){
    app.use(path, that.plugins[path]);
  });
